...
const DEFAULT_LANYARD = '/lanyard.png';
...
function Band({
  ...
  const cardMap = useMemo(() => {
    ...
    // Keep base atlas for card edges/non-face areas (clip, clamp geometry)
    ...
    // For cover mode: wipe front face so React Bits design is gone (photo covers it fully)
    // For contain mode: keep original card texture as natural border around the photo
    ctx.fillStyle = '#111111';
    if (imageFit === 'cover' && frontImage) {
      ctx.fillRect(FRONT_UV_RECT.x * W, FRONT_UV_RECT.y * H, FRONT_UV_RECT.w * W, FRONT_UV_RECT.h * H);
    }
    // Always wipe back face (will be covered by backImage or stays dark)
    ...
    if (frontText) {
      ...
      const overlayH = rh * 0.26;
      const grad = ctx.createLinearGradient(0, ry + rh - overlayH, 0, ry + rh);
      grad.addColorStop(0, 'rgba(0,0,0,0)');
      grad.addColorStop(1, 'rgba(0,0,0,0.88)');
      ctx.fillStyle = grad;
      ctx.fillRect(rx, ry + rh - overlayH, rw, overlayH);

      // Split on " and " → two lines
      const lines = frontText.split(' and ');
      const fontSize = Math.max(12, Math.floor(rw * 0.062));
      const lineHeight = fontSize * 1.45;
      const totalH = lines.length * lineHeight;
      const startY = ry + rh - overlayH / 2 - totalH / 2 + lineHeight * 0.5;
    ...
    return composite;
  }, [frontImage, backImage, frontText, imageFit, frontTex, backTex, materials.base.map]);
